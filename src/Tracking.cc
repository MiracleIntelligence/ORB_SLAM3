...
namespace ORB_SLAM3
{
    ...
    Tracking::Tracking(System *pSys,
        ...
        : mState(NO_IMAGES_YET), mSensor(sensor), mTrackedFr(0), mbStep(false), mbOnlyTracking(false), mbMapUpdated(false), mbVO(false), mpORBVocabulary(pVoc), mpKeyFrameDB(pKFDB), mbReadyToInitializate(false), mpSystem(pSys), mpViewer(NULL), bStepByStep(false), mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpAtlas(pAtlas), mnLastRelocFrameId(0), time_recently_lost(5.0), mnInitialFrameId(0), mbCreatedMap(false), mnFirstFrameId(0), mpCamera2(nullptr), mpLastKeyFrame(static_cast<KeyFrame *>(NULL))
    {
    ...
    void Tracking::TrackStats2File()
    {
        ...
        for (int i = 0; i < vdTrackTotal_ms.size(); ++i)
        {
            ...
            f << stereo_rect << "," << resize_image << "," << vdORBExtract_ms[i] << "," << stereo_match << ","
              << imu_preint << "," << vdPosePred_ms[i] << "," << vdLMTrack_ms[i] << "," << vdNewKF_ms[i] << ","
    ...
    void Tracking::Track()
    {
        ...
        else
        {
            ...
            double timePosePred = std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(
            ...
            if (bOK || mState == RECENTLY_LOST)
            {
                ...
                double timeNewKF = std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(
    ...
    void Tracking::MonocularInitialization()
    {
        ...
        else
        {
            ...
            if (mpCamera->ReconstructWithTwoViews(
                    mInitialFrame.mvKeysUn, mCurrentFrame.mvKeysUn, mvIniMatches, Tcw, mvIniP3D, vbTriangulated))
            {
    ...
    bool Tracking::TrackWithMotionModel()
    {
        ...
        int nmatches = matcher.SearchByProjection(
            mCurrentFrame, mLastFrame, th, mSensor == System::MONOCULAR || mSensor == System::IMU_MONOCULAR);
        ...
        if (nmatches < 20)
        {
            ...
            nmatches = matcher.SearchByProjection(
                mCurrentFrame, mLastFrame, 2 * th, mSensor == System::MONOCULAR || mSensor == System::IMU_MONOCULAR);
    ...
    void Tracking::SearchLocalPoints()
    {
        ...
        if (nToMatch > 0)
        {
            ...
            int matches = matcher.SearchByProjection(
                mCurrentFrame, mvpLocalMapPoints, th, mpLocalMapper->mbFarPoints, mpLocalMapper->mThFarPoints);
        }
    }
    ...
    void Tracking::UpdateLocalPoints()
    {
             ...
             ++itKF)
        {
            ...
            for (vector<MapPoint *>::const_iterator itMP = vpMPs.begin(), itEndMP = vpMPs.end(); itMP != itEndMP;
                 itMP++)
            {
